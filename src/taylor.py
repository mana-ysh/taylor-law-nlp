...
class TaylorLowRegressor():
    ...
    def fit(self, word_seq, width=5620):
        print("processing text...")
        word2posit = defaultdict(list)
        ...
        posit_list = []
        # FIXME: slow
        for w, posits in word2posit.items():
            freqs = []
            for j in range(len(word_seq) // width + 1):
                start_idx = j * width
                end_idx = (j+1) * width
                freqs.append(len(np.where((np.array(posits) >= start_idx) & (np.array(posits) < end_idx))[0]))
            word_list.append(w)
